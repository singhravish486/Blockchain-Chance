...
const WheelGame = () => {
  ...
  const generateSegments = (risk) => {
    ...
    for (let i = 0; i < SEGMENTS; i++) {
      ...
      newSegments.push({
        id: i,
        value: selectedMultiplier.value,
  ...
  const spin = async () => {
    ...
    for (let i = 0; i < segments.length; i++) {
      const multiplier = multiplierConfigs[riskLevel].find(
        (mult) =>
          mult.value === segments[i].value && mult.color === segments[i].color
      );
        ...
        <div className="bet-control">
          ...
          <div className="bet-input-group">
            <input
              ...
              value={bet}
              onChange={(e) =>
                setBet(Math.max(0.0001, Number(e.target.value) || 0.01))
              }
              className="bet-input"
              disabled={isSpinning}
            />
            <button
              onClick={() => setBet((b) => Math.max(0.01, Math.floor(b / 2)))}
            ...
            {["Low", "Medium", "High"].map((level) => (
              <button
                ...
                className={`risk-button ${
                  riskLevel === level ? level.toLowerCase() : ""
                }`}
                disabled={isSpinning}
              >
                {level}
            ...
            {showCongrats && (
              <>
                ...
                >
                  ...
                  >
                    <motion.h2 className="congrats-title">YOU WIN!</motion.h2>
                  </motion.div>
                </motion.div>
                ...
                {confetti.map((particle) => (
                  <motion.div
                    ...
                    animate={{
                      ...
                      rotate:
                        particle.rotation * (Math.random() > 0.5 ? -1 : 1),
                      opacity: [1, 1, 0],
                    }}
                    transition={{
